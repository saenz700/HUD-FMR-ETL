...
# Retrieve fmrs by state, and create a fmrs_state DF.
...
zips_from_fmr_cbsa = list(fmr_cbsa_df['zip_code'].unique())

# Function to fetch zip code data
async def fetch_zip_code_data(zip, year, quarter):
    ...
    if 'data' in response:
        ...
        return [parent_dict.copy() | entry for entry in results]

    return []

# Function to fetch data for a zip code until it finds a match
async def fetch_data_for_zip(zip):
    for year in years_for_zips:
        for quarter in range(4, 0, -1):  # Start with quarter 4 and go backwards
            data = await fetch_zip_code_data(zip, year, quarter)
            if data:
                return data
...
async def main():
    ...
    for zip in zips_from_fmr_cbsa:
        data = await fetch_data_for_zip(zip)
        zip_code_raw.extend(data)
