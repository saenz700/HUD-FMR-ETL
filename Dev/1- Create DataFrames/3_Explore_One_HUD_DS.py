...
state_df = pd.DataFrame(requests.get(hud_base_url+hud_available_apis['all_states'],headers=hud_headers).\
                        json())
unique_states_list = list(state_df['state_code'].unique())

county_list = []
for state in unique_states_list:
    resp = requests.get(hud_base_url+hud_available_apis['all_counties']+str(state),headers=hud_headers)
    county_list = county_list + resp.json()
